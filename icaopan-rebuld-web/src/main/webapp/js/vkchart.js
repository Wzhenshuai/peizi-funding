function VKChart(contentId) {
		...
		this.options = {
			...
			//线条颜色
			priceLineColor: '#F0F0F0',
			middleLineColor: '#F0F0F0',
			otherSplitLineColor: '#F0F0F0',
			borderColor: '#F0F0F0',
			tipColor: '#a3c4e7',
			//水平线与垂直线的条数
...
//			scalersRight.push(percent.toFixed(2) + '%');
	...
	paintKline: function() {
		...
		vkChart.ctx.lineWidth = 1;
		vkChart.ctx.moveTo(vkChart.options.region.x, vkChart.options.region.height / 2 + vkChart.options.region.y);
		var barWidth = vkChart.options.region.width / vkChart.data.items.length * 0.8-2;
		$.each(vkChart.data.items, function(i, item) {
			var x = xSpace * i;
...
//			scalersRight.push(percent.toFixed(2) + '%');
	...
	paintTopText: function(index) {
		...
		var open = data.items[index].preclose;
//		var time = VKChart.Util.getKlineTimeString(this.options.dateTimeType, );
		var time=DF.formateDateToShowDateMMDD(new Date(data.items[index].date).getTime());
		...
		var oneWidth = Math.floor(options.region.width / 7);
		...
		ctx.textAlign = "left";
		topText=time+" ";
		topText+="开:"+data.items[index].open.toFixed(2)+" ";
		topText+="高:"+data.items[index].high.toFixed(2)+" ";
		topText+="低:"+data.items[index].low.toFixed(2)+" ";
		topText+="收:"+data.items[index].close.toFixed(2)+" ";
		topText+="量:"+(data.items[index].volume/10000).toFixed(2)+"万"+" ";
		topText+="幅:"+(isRise ? "↑" : (isFall ? "↓" : "")) + (diff * 100 / open).toFixed(2) + "%";
