...
//			$('ul.nav-tab li:first').attr('class', 'active');
        ...
        function markSelected(obj){
			var newColor = $(obj).css('background-color');
            $(obj).css("background-color","LightSteelBlue");
            var markId =$(obj).find(":radio[name=userId]").val();
            if (lastColor[markId]==undefined){
                lastColor[markId] = newColor;
			}
            if(lastSelect!=null&&markId!=lastSelect){
                $(":radio[value='"+lastSelect+"']").parent().parent('tr').css("background-color",lastColor[lastSelect]);
            }
            lastSelect = markId;
        }
		...
		<div class="table-responsive">
			<div class="row">
				...
				<div class="col-md-4" style="float:right;">
					...
					<button class="btn btn-primary" onclick="allSell()">全部卖出</button>
					<button class="btn btn-primary" id="allRevokeBtn" onclick="allRevoke(this)">全部撤单</button>
				</div>
												...
												<form id="positionForm" method="post">
													<div class="form-group">
														<div class="col-sm-4" style="float:right;">
															...
															<input type="button" onclick="showCommonPlacementModal()" class="btn btn-danger btn-sm" value="普通卖出"/>
														</div>
													</div>
													<table:table id="positionTable" ajaxURI="${ctx}/risk/queryPositionByPage"
																 columnNames="userName,userName,securityCode,securityName,marketValue,marketProfit,amount,availableAmount,latestPrice,costPrice,suspensionDisplay"
																 headerNames="选择,用户名,股票代码,股票简称,市值,浮动盈亏, 持仓数量, 当前可用, 现价, 成本价,停牌标识"
																 searchInputNames="userIdParam"
																 pageLength="6"
																 makeHeadDefsHtmlFunc="positionColumnDefs"
																 showProgress="false"
													/>
                                            ...
                                            <div class="tab-pane" id="1a">
                                                <form id="placementForm" method="post">
                                                    ...
                                                    <table:table id="placementTable" ajaxURI="${ctx}/risk/queryPlacementByPage"
                                                                 columnNames="securityCode,securityCode,securityName,createTimeStr,tradeTypeDisplay,placementPrice,placementQty,placementAmount,filledQty,placementStatusDisplay"
                                                                 headerNames="选择,股票代码,股票名称,委托时间,方向,委托价格,委托数量,委托金额,成交数量,委托状态"
                                                                 searchInputNames="userIdParam"
                                                                 pageLength="6"
                                                                 makeHeadDefsHtmlFunc="placementColumnDefs"
                                                                 loadEnd="LoadEnd()"
                                                                 showProgress="false"
                                                    />
						...
						<div class="modal-dialog" role="document">
							<div class="modal-content">
								...
								<div class="modal-body">
									...
									委托价格: <input type="radio" name="speedPriceType" value="1"/> 买1价
