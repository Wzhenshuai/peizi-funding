...
MS.sellStockCodeInputBlur = function (stockCodeId) {
	...
	var cnSelList =[]; 
	$("input.channelCB:checked").each(function(){ 
		cnSelList.push($(this).val()); 
	}); 
	console.log(cnSelList);
	$.ajax({
		url:systemConfig.path+"/trade/QuerySellStockChannel.ajax", 
		data:{"stockCode":stockCode}, 
		type:'post',
		async:false, 
		success: function(msg){
			...
			for(var i=0;i<msg.length;i++){
				//ops+="<li style=\"height:29px;line-height: 29px;\"><input class=\"channelCB\" type=\"checkbox\" value="+msg[i].channelId+"></input>"+msg[i].channelId+" 可用："+msg[i].amount+"</li>"
				ops+="<tr><td class=\"cb\"><input class=\"channelCB\" type=\"checkbox\" value="+msg[i].channelId+" ";
				if($.inArray(msg[i].channelId+"", cnSelList)!=-1){
					ops+="checked=\"checked\"";	
				}
				ops+="></input></td>";
				ops+="<td>";
				ops+="<table class=\"channelTB\">";
				ops+="<tr><td>通道:</td><td>"+msg[i].channelName+"</td></tr>";
				ops+="<tr><td>余额:</td><td>"+msg[i].amount+"</td></tr>";
				ops+="</table>";
				ops+="</td></tr>";
			}
			$("#channelTB").html(ops);
...
MS.sellStocksChannelChange=function(){
	/* 计算可卖股票数量 */
    calcMaxStocksAmountToSell();
}
...
MS.doBuyPlacementStocksTrade = function (){
	...
	var channelId=[];
	$.each($('input.channelCB:checked'),function(){
	...
	ShowDialogConfirm("确认买入信息", text, "确认买入", function(){
		var txt="";
		doTrade(stockCode,price,quantity,'0',txt,channelId);
	}, "取消", null);
};
...
MS.doSellPlacementStocksTrade = function (){
	...
	ShowDialogConfirm("确认卖出信息", text, "确认卖出", function(){
		var txt="";
		doTrade(stockCode,price,quantity,'1',txt,channelId);
	}, "取消", null);
};
		...
		success:function(r) {
			...
			if (list != null && list.length > 0) {
				...
				for (var i = 0; i < list.length; i++) {
					...
					html+='<a class="btn4 btn4-red" href='+systemConfig.path+"/trade/BuyStocksInitSenior.do?stockCode="+securityLocalCode+'>买入</a>';
					html+='<a class="btn4 btn4-blue mar-t5" href='+systemConfig.path+"/trade/SellStocksInitSenior.do?stockCode="+securityLocalCode+'>卖出</a>';
					html += "</td></tr>";
					isOdd = !isOdd;
				};
			} else {
...
$(function(){
	...
	var lastChannelId=$("#lastChannelId").val();
	if(lastChannelId!=""){
		$("#channelSelect").val(lastChannelId);
	}
});
